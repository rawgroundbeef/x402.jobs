...
- **D-08 (Grandfathering test):** Vitest integration test against in-memory Inngest harness. Test name: "grandfathers in-flight job: run with old fee snapshot routes payment to old URL with old percentage". Co-located at `apps/api/src/inngest/functions/run-workflow/__tests__/grandfather-fee.test.ts` (path corrected during planning per RESEARCH §6 — the `inngest/functions/__tests__/` directory does not exist; the closest analog `escrow.test.ts` lives under `run-workflow/__tests__/`, so the new test lands there alongside it). Models the same pattern as `apps/api/src/inngest/functions/run-workflow/__tests__/escrow.test.ts`.
