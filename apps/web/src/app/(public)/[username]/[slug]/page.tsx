...
export async function generateMetadata({
  ...
  const job = await getJob(cleanUsername, slug);

  const jobPath = `@${job?.owner_username || cleanUsername}/${job?.slug || slug}`;
...
export default async function Page({ params }: PageProps) {
  const { username, slug } = await params;
  // Remove @ prefix if present (URL will be /@username/slug)
  const cleanUsername = username.startsWith("@") ? username.slice(1) : username;
  return <PublicJobPage username={cleanUsername} slug={slug} />;
}
