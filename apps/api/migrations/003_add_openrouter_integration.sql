...
CREATE TABLE IF NOT EXISTS x402_user_openrouter_integrations (
...
CREATE INDEX idx_openrouter_integrations_user_enabled
  ON x402_user_openrouter_integrations(user_id, is_enabled);
...
CREATE TRIGGER openrouter_integration_updated_at
  BEFORE UPDATE ON x402_user_openrouter_integrations
  FOR EACH ROW
...
ALTER TABLE x402_resources ADD COLUMN IF NOT EXISTS openrouter_model_id UUID REFERENCES x402_openrouter_models(id) ON DELETE SET NULL;

-- Config for OpenRouter execution (model params, system prompt, etc.)
ALTER TABLE x402_resources ADD COLUMN IF NOT EXISTS openrouter_config JSONB;
...
CREATE INDEX IF NOT EXISTS idx_x402_resources_openrouter_model
  ON x402_resources(openrouter_model_id)
...
-- -- Drop trigger
-- DROP TRIGGER IF EXISTS openrouter_integration_updated_at ON x402_user_openrouter_integrations;

-- -- Drop trigger function
-- DROP FUNCTION IF EXISTS update_openrouter_integration_updated_at();

-- -- Drop policies
-- DROP POLICY IF EXISTS "Service role full access on OpenRouter" ON x402_user_openrouter_integrations;
-- DROP POLICY IF EXISTS "Users can manage their own OpenRouter config" ON x402_user_openrouter_integrations;
