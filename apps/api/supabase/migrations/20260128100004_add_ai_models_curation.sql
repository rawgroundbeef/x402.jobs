...
ALTER TABLE public.x402_openrouter_models
...
CREATE INDEX IF NOT EXISTS idx_x402_openrouter_models_modality ON public.x402_openrouter_models(modality);

-- Partial index on is_curated for curated view (only index true values)
CREATE INDEX IF NOT EXISTS idx_x402_openrouter_models_curated ON public.x402_openrouter_models(is_curated) WHERE is_curated = true;
...
-- Drop constraint
ALTER TABLE public.x402_openrouter_models DROP CONSTRAINT IF EXISTS check_modality_valid;

-- Drop columns
ALTER TABLE public.x402_openrouter_models DROP COLUMN IF EXISTS is_curated;
ALTER TABLE public.x402_openrouter_models DROP COLUMN IF EXISTS modality;
*/
